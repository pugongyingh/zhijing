...
(function () {
    ...
    function o(h, n) { for (var B = [], x = 0; x < h.length; x++) B.push(h[x]); for (x = 0; x < B.length; x++) n(B[x]) } function v() { o([e, m], function (h) { o(A.getElementsByTagName(h), function (n) { var B = true; if (n.canPlayType) if (n.src && n.canPlayType(F(h, n.src))) B = false; else o(n.getElementsByTagName("source"), function (x) { if (n.canPlayType(F(h, x.src, x.type))) B = false }); B && v.createFallback(h, n) }) }) } function y(h) { return h.split("index.html").slice(0, -1).join("index.html") + "/" } function F(h, n, B) { return B || k[h][n.split(".").slice(-1)[0]] || G[h] }
    function H(h, n) { h = h.getAttribute(n); return h == true || typeof h == "string" } function z(h) { if (h.substr(0, 1) == "index.html") return s + h; if (h.substr(0, 1) == "." || !h.match(/^\s*\w+:\/\//)) return w + h; return h } function D(h, n, B) { var x = h.getAttribute(n); if (x) return x + "px"; if (h.currentStyle) h = h.currentStyle[n]; else if (C.getComputedStyle) h = A.defaultView.getComputedStyle(h, null).getPropertyValue(n); else return B; if (h == "auto") return B; return h } function r(h) { return h.match(/\s*([\w-]+\/[\w-]+);|\s/)[1] } function b(h, n) {
        return r(h) ==
        r(n)
